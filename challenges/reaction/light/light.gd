...
func _ready() -> void:
	...
	GameManager.button_down.connect(func():
		if react_time_started:
			...
			GameManager.on_game_success('Reaction time: {time_string} sec.'.format({'time_string': str(time_string)}))
			if timer.timeout.is_connected(timeout):
				timer.timeout.disconnect(timeout)
		else:
			is_light_on = !is_light_on
			if is_light_on:
				GameManager.on_game_failed('You clicked too fast.')
				if timer.timeout.is_connected(timeout):
					timer.timeout.disconnect(timeout)
			else:
				GameManager.on_game_reset('Click the button when the light lights up.')
				react_time = 0.0
